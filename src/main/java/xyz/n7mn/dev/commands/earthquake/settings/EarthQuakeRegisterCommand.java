...
import xyz.n7mn.dev.managers.search.entity.EntitySelectMenuInteract;
import xyz.n7mn.dev.managers.search.entity.UseEntitySelectMenu;
...
public class EarthQuakeRegisterCommand extends SlashCommandListener {
    ...
    public void onEarthQuakeOverride(ButtonInteractionEvent event) {
        if (!event.getMember().hasPermission(Permission.ADMINISTRATOR)) {
            event.replyEmbeds(getErrorEmbeds("あなたは権限を持っていません (管理者権限が必要です)").build()).setEphemeral(true).queue();
            return;
        }
    ...
    @EntitySelectMenuInteract(regex = "choose-earthquake-channel")
    public void onEarthQuakeEntitySelect(EntitySelectInteractionEvent event) {
        //常に大丈夫なはずだけど、一応確認する必要はある
        MessageChannelUnion channel = (MessageChannelUnion) event.getValues().get(0);
        if (!(event.getChannel() instanceof StandardGuildMessageChannel standard)) {
            event.replyEmbeds(getErrorEmbeds("登録できないチャンネルタイプです").build()).queue();
            return;
        }
        EarthQuakeDB.EarthQuakeData data = SQLite.INSTANCE.getEarthQuake().get(event.getGuild().getId());
